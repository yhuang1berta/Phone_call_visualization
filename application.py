...
def process_event_history(log: Dict[str, List[Dict]],
    ...
    """
    d = datetime.datetime.strptime(log['events'][0]['time'],
                                   "%Y-%m-%d %H:%M:%S")
    current_date = [d.month, d.year]
    for e in log['events']:
        billing_date = datetime.datetime.strptime(e['time'],
                                                  "%Y-%m-%d %H:%M:%S")
        if billing_date.year > current_date[1]:
            new_month(customer_list, e['time'].month, e['time'].year)
            current_date = [e['time'].month, e['time'].year]
        elif billing_date.month > current_date[0] \
                and billing_date.year == current_date[1]:
            new_month(customer_list, billing_date.month, billing_date.year)
            current_date = [billing_date.month, billing_date.year]

        if e['type'] == 'call':
            new_call = Call(e['src_number'],
                            e['dst_number'],
                            billing_date,
                            e['duration'],
                            e['src_loc'],
            ...
            caller = find_customer_by_number(new_call.src_number, customer_list)
            caller.make_call(new_call)
            receiver = find_customer_by_number(new_call.dst_number,
                                               customer_list)
            receiver.receive_call(new_call)
