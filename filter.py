    ...
    def apply(self, customers: List[Customer],
        ...
        try:
            if filter_string != filter_string.strip():
                ...
                filter_id = int(filter_string)
                valid = True
        except ValueError:
            return data
        for cust in customers:
            if valid and cust.get_id() == filter_id:
                phone_line = cust.get_phone_numbers()
                for num in data:
                    if num.src_number in phone_line \
                            or num.dst_number in phone_line:
...
class DurationFilter(Filter):
    ...
    def apply(self, customers: List[Customer],
        ...
        valid = False
        try:
            duration = int(filter_string[1:4])
        ...
        for num in data:
            dur = num.duration
            if valid and t == 'L' and dur < duration:
                return_list.append(num)
            elif valid and t == 'G' and dur > duration:
                return_list.append(num)
        return return_list
...
class LocationFilter(Filter):
    ...
    def apply(self, customers: List[Customer],
        ...
        return_list = []
        valid = False
        parse = filter_string.split(', ')
        try:
            upper = (float(parse[2]), float(parse[3]))
            lower = (float(parse[0]), float(parse[1]))
            valid = True
        except (IndexError, ValueError, TypeError):
            return data
        for num in data:
            if valid:
                if lower[0] <= num.src_loc[0] <= upper[0]:
                    if lower[1] <= num.src_loc[1] <= upper[0]:
                        return_list.append(num)
                elif lower[0] <= num.dst_loc[0] <= upper[0]:
                    if lower[1] <= num.dst_loc[1] <= upper[1]:
                        return_list.append(num)
        return return_list
